...
class OfflineQueue {
  ...
  private saveToStorage() {
    try {
      ...
      const createQueueForStorage = this.createQueue.map(item => ({
        ...
        data: {
          ...item.data,
          images: item.data.images ? item.data.images.map(file => ({
            name: file.name,
            type: file.type,
            size: file.size,
  ...
  queueTodoUpdate(todoId: string, data: { title: string; description?: string; completed?: boolean; images?: File[]; remove_images?: string[] }): string {
