...
import type { ChatMessage, ChatRecentResponse, MessageSentEvent, UserTypingEvent } from '@/types';
                    ...
                    <ScrollArea className="mb-4 flex-1" ref={scrollRef}>
                        <div className="space-y-3 p-2">
                            {isLoading ? (
                                ...
                                <div className="py-8 text-center text-muted-foreground">No messages yet. Be the first to say hello! 👋</div>
                            ) : (
                                messages.map((msg: ChatMessage) => (
                                    <div key={msg.id} className={`flex ${msg.is_own ? 'justify-end' : 'justify-start'}`}>
                                        <div
                                            className={`max-w-[70%] rounded-lg px-3 py-2 ${
                                                msg.is_own ? 'bg-primary text-primary-foreground' : 'bg-muted'
                                            }`}
                                        >
