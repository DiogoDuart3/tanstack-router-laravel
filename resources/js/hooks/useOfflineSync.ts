    ...
    const syncAction = async (action: QueuedAction) => {
        try {
            switch (action.type) {
                case 'create': {
                    ...
                    if (response.ok) {
                        ...
                        setTodos((prev) =>
                            prev.map((t) =>
                                t.id === action.todoId ? { ...t, status: 'synced', serverId: serverTodo.id, imageUrl: serverTodo.imageUrl } : t,
                            ),
                        );
            ...
            if (action.type === 'create') {
                setTodos((prev) =>
                    prev.map((t) =>
                        t.id === action.todoId ? { ...t, status: 'error', error: error instanceof Error ? error.message : 'Sync failed' } : t,
                    ),
                );
            }

            setSyncQueue((prev) => prev.map((a) => (a.id === action.id ? { ...a, retryCount: a.retryCount + 1, lastAttempt: Date.now() } : a)));
        }
    };
