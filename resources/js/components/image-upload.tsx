        ...
        for (let i = 0; i < newFiles.length; i++) {
            ...
            try {
                ...
                if (compressionRatio > 0.1) {
                    // If compressed by more than 10%
                    toast.success(
                        `${file.name} compressed from ${(file.size / 1024 / 1024).toFixed(1)}MB to ${(compressedFile.size / 1024 / 1024).toFixed(1)}MB`,
                    );
                } else {
            ...
            >
                <div className="flex flex-col items-center gap-2">
                    ...
                    <div>
                        <p className="text-sm font-medium">
                            {isCompressing ? (
                                <div className="space-y-1">
                                    <div>Compressing {currentProcessingFile ? `"${currentProcessingFile}"` : 'images...'}</div>
                                    {compressionProgress.total > 1 && (
                                        <div className="space-y-1">
                                            <div className="text-xs text-muted-foreground">
                                                {compressionProgress.current} of {compressionProgress.total} images
                                            </div>
                                            <div className="h-1.5 w-48 rounded-full bg-gray-200 dark:bg-gray-700">
                                                <div
                        ...
                        <p className="text-xs text-muted-foreground">Up to {maxImages} images, automatically compressed to 1MB each</p>
                    </div>
                </div>
                <Input
                    ref={fileInputRef}
